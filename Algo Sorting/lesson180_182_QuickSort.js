...
function partition(arr, l, r) {
  ...
  //const testArr = [5,3,2,1,0];
  let pivot = arr[l];
  let i = l;
  let j = r;

  while (true) {
    //keep in the loop until  j< i,
    //then we know we found the j sorted position

    //scanning with i and j; you want i to increment 1st, then do the "while";
    //this way, when it reaches the end, it will not be bigger than the array's length;
    do {
      i++;
...
function quickSort(arr, l = 0, h = arr.length - 1) {
  ...
  //Or bassically when l=h in the nested quickSort functions.
  //If that is the scenario we hit, it means the elem is in the sorted position;
  //So the quickSort fct does not need to do anything;

  //While the subarray we want to sort is at least 2 elems, we do recursion:

  if (l < h) {
    let sortedPos = partition(arr, l, h);

    //don't take into account the sorted position when creating the 2 unsorted subarrays;
    quickSort(arr, l, sortedPos - 1); 
    quickSort(arr, sortedPos + 1, h);
    return arr;
  }
}

const testArr = [10, 16, 8, 12, 15, 6, 3, 9, 5];
...
//Space complexity = O (log n) ( better than Merge sort)
